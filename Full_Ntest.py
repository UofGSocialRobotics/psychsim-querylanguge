...
from modified_psychsim.pwl import *
from modified_psychsim.reward import *
from modified_psychsim.action import Action,ActionSet
...
#                                   False: False}))
    ...
    # Dynamics of offers
        ...
        #
                ...
                self.world.setDynamics(stateKey(None,'%sOffer' % (fruit)) ,atom,tree)
    ...
    # Dynamics of agreements
        for i in range(2):
            ...
            self.world.setDynamics(stateKey(atom['subject'],'agree'),atom,tree)
        ...
        # rejecting give us batna and ends negotiation
            ...
            self.world.setDynamics(stateKey(atom['subject'],'BatnaOwned') ,atom,tree)

            tree = makeTree(setToFeatureMatrix(stateKey(atom['object'],'BatnaOwned') ,stateKey(atom['object'], 'Batna')))
            self.world.setDynamics(stateKey(atom['object'],'BatnaOwned') ,atom,tree)

            tree = makeTree(setTrueMatrix(stateKey(None,'rejectedNegotiation')))
            self.world.setDynamics(stateKey(None,'rejectedNegotiation' ),atom,tree)
