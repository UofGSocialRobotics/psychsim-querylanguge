...
class World:
    ...
    #
    # def lo_explain(self,outcomes,level=5,buf=None):
    #     """
    #     Generate a more readable interpretation of outcomes generated by L{step}
    #     @param outcomes: the return value from L{step}
    #     @type outcomes: dict[]
    ...
    #     """
    #     for outcome in outcomes:
    #         if level > 0: psychlogs.log.info('%d%%' % (outcome['probability']*100.))
    #         if outcome.has_key('actions'):
    #             self.explainAction(outcome,buf,level)
    #             for name,action in outcome['actions'].items():
    #                 if not outcome['decisions'].has_key(name):
    #                     # No decision made
    #                     if level > 1: psychlogs.log("\tforced") # print >> buf,'\tforced'
    #                 elif level > 1:
    #                     # Explain decision
    #                     self.explainDecision(outcome['decisions'][name],buf,level)
            ...
            if level > 2:
                ...
                for state in beliefs:
                    ...
                    while len(nodes) > 0:
                        ...
                        # print("%d = %d - %d" % (t, V[state]['horizon'], node['horizon']))
            ...
            if not entity is None:
                ...
                if vector.has_key(key):
                    if csv:
                        ...
                        print >> buf,'\t%12s' % (''),
                        self.agents[entity].printModel(index=vector[key],prefix=prefix,buf=buf)
                    newEntity = False
